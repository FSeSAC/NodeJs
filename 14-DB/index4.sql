...
-- SQL에서 기본적으로 join 키워드를 사용하거나 ,를 통해 연결하면 inner join 연산을 수행
-- 이 때, 조건을 설정되지 않은 inner join 은 cross join 연산
...
-- full outer join
-- mysql에서느 full outer join 지원 X
-- union이라는 키워드를 통해 full outer join 구현 가능
